...
os.environ.setdefault("YOLO_CONFIG_DIR", _APP_ROOT)
...
def _resolve_car_weights() -> str:
    ...
    Prefer the explicit path you use on Windows; otherwise use ./models beside this file
    (Docker / Hugging Face Spaces).
    """
    preferred = (
        r"C:\Users\Mahmoud Magdy\Downloads\drive-download-20260510T155354Z-3-001"
        r"\huggingface-space\models\yolo8_last_version_car(must).pt"
    )
    rel = os.path.join(_APP_ROOT, "models", "yolo8_last_version_car(must).pt")
    for p in (preferred, rel):
        if os.path.isfile(p):
...
def _resolve_plate_weights() -> str:
    preferred = (
        r"C:\Users\Mahmoud Magdy\Downloads\drive-download-20260510T155354Z-3-001"
        r"\huggingface-space\models\plattes_detect_model.pt"
    )
...
def _resolve_ocr_weights() -> str:
    preferred = (
        r"C:\Users\Mahmoud Magdy\Downloads\drive-download-20260510T155354Z-3-001"
        r"\huggingface-space\models\best.pt"
    )
            ...
            with c2:
                ...
                if triple_mode:
                    ...
                    if car_only:
                        ...
                        types_unique = sorted(
                            {str(d["vehicle_type"]) for d in dets}
                        )
                        st.markdown(
                            f"**Vehicle types:** "
                            f"`{', '.join(types_unique) if types_unique else '—'}`"
